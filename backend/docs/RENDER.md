...
**Importante (subida de imágenes en la app):** si `MINIO_PUBLIC_URL` apunta a `http://localhost:9000`, la API devuelve URLs inútiles para el móvil y las subidas fallan. En producción debes usar **HTTPS** y un host público (S3/R2/CDN). La app sube por `POST /uploads/direct` (HTTPS → API); el servidor escribe en el bucket.
...
MINIO_BUCKET=drinkquest
MINIO_PUBLIC_URL=https://<tu-cdn-o-dominio-publico>/drinkquest
```
